...
vector<node*> generateMoves(const vector<vector<char>>& grid, node* currentNode) {
    ...
    // Clear the current node's neighbors and set it to the newly generated neighbors
    for (node* neighbor : currentNode->neighbors) {
        delete neighbor;
    }
    currentNode->neighbors = nodes;
...
node* IDAStarSearch(node* startNode, node* goalNode, int threshold) {
    map<node*, bool> visited; // Change the map to store node* instead of board configurations
    ...
    while (true) {
        ...
        while (!openList.empty()) {
            ...
            int fValue = currentNode->g + Manhattan_Start_distance(currentNode, goalNode);

            if (fValue > threshold) {
                break; // Exceed the threshold, exit this iteration
            }

            if (currentNode == goalNode) {
                return currentNode; // Goal node found, return the solution
            }

            visited[currentNode] = true; // Mark the node as visited

            vector<node*> nextMoves = generateMoves(currentNode->board, currentNode);
            for (node* nextMove : nextMoves) {
                ...
                if (visited.find(nextMove) == visited.end() || nextMove->f <= threshold) {
        ...
        threshold = openList.top()->f;
    }

    return nullptr; // Solution not found (this should not be reached)
}
...
int main() {
  ...
  // string name = "15#764238";
   //string goal = "82#547361";
    ...
    createNode(goalNode, goal_Grid);
    createNode(startNode, grid);

    int threshold = Manhattan_Start_distance(startNode, goalNode);

    while (true) {
        node* foundSolution = IDAStarSearch(startNode, goalNode, threshold);
        if (foundSolution != nullptr) {
